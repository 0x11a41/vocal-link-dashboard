...
export function EnhancePanel(rid: string): HTMLElement {
  ...
  function updateOpts(isChecked: boolean): void {
    if (isChecked) {
      increment();
    } else {
      decrement();
...
function createOption({label, btn}: createOptionProps): HTMLElement {
...
async function enhance(rid: string, props: number): Promise<boolean> {
  try {
